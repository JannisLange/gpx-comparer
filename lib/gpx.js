...
export function parseGpx(xmlText) {
  if (typeof xmlText !== 'string' || !xmlText.trim()) throw new Error('The uploaded GPX file is empty.');
  if (XMLValidator.validate(xmlText) !== true) throw new Error('The uploaded file is not valid GPX/XML.');
  let document;
  try {
  ...
  if (!document?.gpx || typeof document.gpx !== 'object') throw new Error('The uploaded XML file is not a GPX document.');
...
export function calculateTripMetrics(points) {
  if (points.some((point) => !Number.isFinite(point.time))) throw new Error('Every GPX track point must have a valid timestamp.');
  let distanceM = 0;
  let movingTimeS = 0;
  let maxSpeedMps = 0;
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    const segmentTimeS = Number.isFinite(previous.time) && Number.isFinite(current.time)
      ...
      : NaN;
    if (segmentTimeS <= 0) throw new Error('GPX track-point timestamps must increase throughout the ride.');
  ...
  const elapsedTimeS = Math.max(0, (points.at(-1).time - points[0].time) / 1000);
  if (elapsedTimeS <= 0) throw new Error('The GPX timestamps do not describe a positive trip duration.');

  return {
    recordedAt: new Date(points[0].time).toISOString(),
    distanceM,
    elapsedTimeS,
    movingTimeS,
